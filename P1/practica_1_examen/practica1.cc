...
int objeto = 1; //1 = tetraedro, 2 = cubo
int modo = 0; //0 = puntos, 1 = alambre, 2 = solido, 3 = ajedrez, 4 = examen
...
void draw_objects()
{
	if (objeto == 1){ //Tetraedro
		...
		else if (modo == 4){ //Circulos Examen
			p.draw_aristas(1.0,0,0,1);
			p.draw_circulos(0.5, 32, GL_FILL);
		}
	}
		...
		if (modo == 0){ //Puntos
			...
			c.draw_aristas(1.0,0,0,1);
			c.draw_circulos(0.5, 32, GL_FILL);
		}
	}
}
...
if (toupper(Tecla1)=='S') modo = 2;   // Solido
if (toupper(Tecla1)=='A') modo = 3;   // Ajedrez
if (toupper(Tecla1)=='C') modo = 4;   // Circulos
